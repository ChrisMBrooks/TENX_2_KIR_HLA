...
if __name__ == '__main__':
    ...
    if not bam_input:
        ...
        for (i, sample), outbam in zip(enumerate(args.input), bam_paths):          
            ...
            CMD_FORMATTED_TXT = MAPPING_CMD % (threads, outbam,
                MAPPING_REF[ref_type], sample
            )
            if VERBOSE:
                print("Razers3 Command: ", CMD_FORMATTED_TXT)

            subprocess.call(CMD_FORMATTED_TXT, shell=True)
